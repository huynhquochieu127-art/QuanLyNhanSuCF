...
export default function ShiftScheduling() {
  ...
  const getWeekRange = () => {
    ...
    return {
      startStr: start.toLocaleDateString('sv-SE', { timeZone: 'Asia/Ho_Chi_Minh' }),
      endStr: end.toLocaleDateString('sv-SE', { timeZone: 'Asia/Ho_Chi_Minh' }),
    };
  };
  ...
  const getDaysInWeek = () => {
    ...
    return dayNames.map((name, i) => {
      ...
      return {
        date: d.toLocaleDateString('sv-SE', { timeZone: 'Asia/Ho_Chi_Minh' }),
        name,
        ...
        {location.pathname !== "/" && (
          <div className="flex border-b border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/50">
            <button
              ...
              className={`px-6 py-4 text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "register"
            ...
            {(isManager || isAdmin) && (
              <button
                ...
                className={`px-6 py-4 text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "approvals"
                    ? "text-sky-600 dark:text-sky-400 border-b-2 border-sky-600 bg-white dark:bg-zinc-900"
                    : "text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-zinc-400 dark:hover:bg-zinc-800"
                  }`}
              >
            ...
            {weekStatus === 'admin_approved' && (
              <button
                ...
                className={`px-6 py-4 text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "official"
                    ? "text-sky-600 dark:text-sky-400 border-b-2 border-sky-600 bg-white dark:bg-zinc-900"
                    : "text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-zinc-400 dark:hover:bg-zinc-800"
                  }`}
              >
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {isStaff && activeTab !== "official" && (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm ${Object.keys(draftChanges).length === 0 || (weekStatus === 'open' && isPastDeadline())
